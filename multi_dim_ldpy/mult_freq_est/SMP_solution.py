...
def SMP_GRR_Client(input_tuple, lst_k, d, epsilon):
    ...
    sanitized_att = (rnd_att, GRR_Client(input_tuple[rnd_att], lst_k[rnd_att], epsilon))
        

    return sanitized_att


def SMP_UE_Client(input_ue_tuple, d, epsilon, optimal=True):
    
    rnd_att = np.random.randint(d)
    
    sanitized_ue_att = (rnd_att, UE_Client(input_ue_tuple[rnd_att], epsilon, optimal))   
    
    return sanitized_ue_att
...
def SMP_UE_Aggregator(reports_ue_tuple, d, epsilon, optimal=True):
    ...
    dic_rep_ue_smp = {att:[] for att in range(d)}
    for val in reports_ue_tuple:
        dic_rep_ue_smp[val[0]].append(val[-1])
    
    lst_freq_est = []
    for idx in range(d):
        
        lst_freq_est.append(UE_Aggregator(dic_rep_ue_smp[idx], epsilon, optimal))
