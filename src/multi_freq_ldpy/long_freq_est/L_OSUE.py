...
def L_OSUE_Client(input_data, k, eps_perm, eps_1):
    ...
    """
    Longitudinal OUE-SUE (L-OSUE) [1] protocol that chains OUE [2] for first round and SUE [2] for second round of sanitization.
...
def L_OSUE_Aggregator_MI(reports, eps_perm, eps_1):
...
def L_OSUE_Aggregator_IBU(reports, k, eps_perm, eps_1, nb_iter=10000, tol=1e-12, err_func="max_abs"):
    """
    Estimator based on Iterative Bayesian Update[3,4].

    :param reports: list of all L-OSUE UE-based vectors;
    :param k: attribute's domain size;
    :param eps_perm: upper bound of privacy guarantee (infinity reports);
    :param eps_1: lower bound of privacy guarantee (a single report), thus, eps_1 < eps_perm;
